...
export default function PsychiatristBookedSessions(props:any) 
{
	...
	const [psychBookedSessions, setPsychBookedSessions] = useState<PsychiatristBookedSession[]>([])
	...
	return (
		<div className="min-h-screen rounded-2xl bg-gray-50 dark:bg-gray-900 p-8">
			<div className="max-w-7xl mx-auto">
				{/* Header */}
				<div className="mb-8 rounded-md bg-gradient-to-r from-gray-200 to-gray-200 dark:from-gray-800 dark:to-gray-700 py-5 px-5">
					<h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Booked Sessions</h1>
					<p className="text-gray-600 dark:text-gray-300">Manage and view all student booking sessions</p>
				</div>
					...
					filteredSessions.length > 0 ? (
						<div className="overflow-x-auto">
							<table className="w-full">
								...
								<tr>
									<th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Student</th>
									<th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Date & Time</th>
									<th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase">Session Type</th>
									...
									filteredSessions.map((session) => (
										...
										<td className="px-6 py-4">
											<div className="flex items-center space-x-3">
												...
												/>
												<div>
													<p className="font-semibold text-gray-900 dark:text-white">{session.studentId.studentName}</p>
													<p className="text-sm text-gray-600 dark:text-gray-400">{session.studentId.studentAdmissionNum}</p>
												</div>
											</div>
										</td>
										<td className="px-6 py-4">
										...
										<td className="px-6 py-4">
											<div>
											<p className="font-medium text-gray-900 dark:text-white">{session.sessionId.sessionType}</p>
											<p className="text-sm text-gray-600 dark:text-gray-400">{session.sessionId.sessionDuration} minutes</p>
											</div>
										</td>
										<td className="px-6 py-4">
											<div className="flex items-center space-x-2">
												{session.sessionId.sessionMode === 'Virtual' && <Video className="w-4 h-4 text-blue-600 dark:text-blue-400" />}
												{session.sessionId.sessionMode === 'In-Person' && <MapPin className="w-4 h-4 text-green-600 dark:text-green-400" />}
												{session.sessionId.sessionMode === 'Phone' && <Phone className="w-4 h-4 text-purple-600 dark:text-purple-400" />}
											<span className="text-gray-900 dark:text-white text-sm">{session.sessionId.sessionMode}</span>
											</div>
										</td>
										<td className="px-6 py-4">
										...
										<td className="px-6 py-4">
											...
											<button
												...
												className="px-4 py-2 bg-blue-600 dark:bg-blue-700 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition font-semibold text-sm"
											>
												View Details
											</button>
											{session.sessionId.status === 'confirmed' && isSessionUpcoming(session.date) && (
												<button className="px-4 py-2 bg-green-600 dark:bg-green-700 text-white rounded-lg hover:bg-green-700 dark:hover:bg-green-600 transition font-semibold text-sm">
													Start Session
												</button>
											)}
											</div>
				...
				showSessionDetails && selectedSession && (
					<div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center p-4 z-50">
						<div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
							{/* Modal Header */}
							<div className="bg-gradient-to-r from-blue-600 to-blue-700 dark:from-gray-800 dark:to-gray-700 text-white p-6 flex items-center justify-between">
								<h2 className="text-2xl font-bold">Session Details</h2>
								<button
									onClick={() => setShowSessionDetails(false)}
									className="p-2 hover:bg-blue-500 dark:hover:bg-gray-600 rounded-full transition"
								>
									<X className="w-6 h-6" />
								</button>
							</div>
							...
							<div className="p-8">
								...
								<div className="mb-6">
									...
									<div className="flex items-center space-x-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
										...
										/>
										<div>
											<p className="text-lg font-bold text-gray-900 dark:text-white">{selectedSession?.studentDetailsInfo?.studentName || ""}</p>
											<p className="text-gray-600 dark:text-gray-400">Admission: {selectedSession.studentId.studentAdmissionNum}</p>
											<p className="text-gray-600 dark:text-gray-400">Age: {selectedSession?.studentDetailsInfo?.studentAge || ""} years</p>
										</div>
									</div>
								</div>
									...
									<div className="space-y-2">
										<div className="flex items-center space-x-3">
											...
											<div>
												...
												<p className="font-semibold text-gray-900 dark:text-white">{selectedSession?.studentDetailsInfo?.phoneNumber}</p>
											</div>
										</div>
										<div className="flex items-center space-x-3">
											...
											<div>
												...
												<p className="font-semibold text-gray-900 dark:text-white truncate">{selectedSession.studentId.email}</p>
											</div>
										</div>
									</div>
								</div>
								...
								{/* Session Details */}
								<div className="mb-6">
									<h3 className="font-bold text-gray-900 dark:text-white mb-3">Session Information</h3>
									<div className="grid md:grid-cols-2 gap-4">
										...
										<div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
											...
											<p className="font-semibold text-gray-900 dark:text-white">{selectedSession.sessionId.sessionType}</p>
										</div>
										<div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
											<p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Duration</p>
											<p className="font-semibold text-gray-900 dark:text-white">{selectedSession.sessionId.sessionDuration} minutes</p>
										</div>
										<div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
								...
								{/* Session Reason */}
								<div className="mb-6">
									<h3 className="font-bold text-gray-900 dark:text-white mb-3">Session Reason</h3>
									<div className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
										<p className="text-gray-700 dark:text-gray-300">{selectedSession.reason} This part is not configured on Backend</p>
									</div>
								</div>
								...
								{selectedSession.notes && (
									<div className="mb-6">
										...
										<div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg">
											<p className="text-gray-700 dark:text-yellow-200">{selectedSession.notes}</p>
										</div>
									</div>
								)}
								...
								<div className="flex space-x-4 pt-6 border-t border-gray-200 dark:border-gray-700">
									<button className="flex-1 px-4 py-3 bg-blue-600 dark:bg-blue-700 text-white font-semibold rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition">
										Send Message
									</button>
									{selectedSession.status === 'confirmed' && isSessionUpcoming(selectedSession.date) && (
										<button className="flex-1 px-4 py-3 bg-green-600 dark:bg-green-700 text-white font-semibold rounded-lg hover:bg-green-700 dark:hover:bg-green-600 transition">
											Start Session
										</button>
									)}
									<button
