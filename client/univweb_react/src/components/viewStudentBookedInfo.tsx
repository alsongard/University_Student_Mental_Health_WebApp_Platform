    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-6">
		<div className="max-w-5xl mx-auto">
			...
			<div className="mb-8">
				...
				<p className="text-gray-600">Manage and view your upcoming therapy sessions</p>
			</div>
			...
			<div className="space-y-6">
				{
					sessions.length === 0 ? (
						<div className="bg-white rounded-2xl shadow-lg p-12 text-center">
							...
							<h3 className="text-xl font-semibold text-gray-800 mb-2">No Booked Sessions</h3>
							...
							<div className="flex items-start justify-between">
								<div className="flex-1">
									<div className="flex items-center gap-3 mb-2">
										...
										<h2 className="text-2xl font-bold">{bookedSession.sessionId.sessionType}</h2>
									</div>
								...
								<div className={`flex items-center gap-2 px-4 py-2 rounded-full border ${getStatusColor(session.status)}`}>
								...
								<div className="space-y-3">
									...
									<div className="flex items-start gap-3">
										...
										<div>
											...
											<p className="font-semibold text-gray-800">{bookedSession.specilization}</p>
										</div>
									</div>

									{session.psychiatristId.phoneNumber && (
										<div className="flex items-start gap-3">
											<Phone className="w-5 h-5 text-gray-400 mt-0.5" />
											<div>
												<p className="text-sm text-gray-600">Contact</p>
												<p className="font-semibold text-gray-800">{session.psychiatristId.phoneNumber}</p>
											</div>
										</div>
									)}

									{session.psychiatristId.officeLocation && (
										<div className="flex items-start gap-3">
											<MapPin className="w-5 h-5 text-gray-400 mt-0.5" />
											<div>
												<p className="text-sm text-gray-600">Office</p>
												<p className="font-semibold text-gray-800">{session.psychiatristId.officeLocation}</p>
											</div>
										</div>
									)}
								</div>
								...
								<div className="space-y-3">
										...
										session.sessionId.location && (
											<div className="flex items-start gap-3">
												...
												<div>
													...
													<p className="font-semibold text-gray-800">{session.sessionId.location}</p>
												</div>
											</div>
									)}
									...
									<div className="flex items-start gap-3">
										...
										<div>
											...
											<p className="font-semibold text-gray-800">{formatDate(session.createdAt)}</p>
										</div>
									</div>
								</div>
							</div>
						...
						{session.notes && (
							<div className="px-6 pb-6">
								<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
									...
									<p className="text-sm text-yellow-700">{session.notes}</p>
								</div>
							</div>
						)}
