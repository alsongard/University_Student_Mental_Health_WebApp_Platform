...
// THE	following works but preferably use new DAte()
const sessionComparison = ()=>{
	const sessionStringDate = "2025-12-31T00:00:00.000Z";
	const sessionDate = new Date(sessionStringDate).toISOString().split("T")[0];
	const todayDate = new Date().toISOString().split("T")[0];
	console.log(`typeof sessionDate: ${typeof(sessionDate)}\ntypeof todayDate: ${typeof(todayDate)}`);

	if (todayDate < sessionDate)
	{
		console.log("today is future from sessionDate");
	}
}
sessionComparison();